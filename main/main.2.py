...
try:
    ...
    helperFunction.periodicalUpdateDatabase()
...
def updateReward(user_id,time,restaurant_id,reward):
    ...
    query = "SELECT FROM recommendation where user_id = %s and restaurant_id = %s"
    ...
    A, b, _ = linUCB(A, b, [result], [], alpha, context_size) #it's learning 
